...
export default class Record{
    ...
    //constructor
	constructor(rec_id, rec_date, rec_lastmodified, rec_dease, rec_desc, rec_indiagnose, rec_outdiagnose, rec_description, rec_conclusion, rec_examineday, rec_reexamineday, appoint_id){
		this.rec_id = rec_id;
    ...
    async InsertRecordToDatabase(){
		...
		var record = await db.get('SELECT * FROM RECORD WHERE rec_id = ?', [this.rec_id]);
		if(record) {
		...
		try {
			var result = await db.run('INSERT INTO RECORD (rec_id, rec_date, rec_lastmodified, rec_dease, rec_desc, rec_indiagnose, rec_outdiagnose, rec_description, rec_conclusion, rec_examineday, rec_reexamineday, appoint_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', 
									[this.rec_id, this.rec_date, this.rec_lastmodified, this.rec_dease, this.rec_desc, this.rec_indiagnose, this.rec_outdiagnose, this.rec_description, this.rec_conclusion, this.rec_examineday, this.rec_reexamineday, this.appoint_id]);
		}
		catch{
    ...
    static async GetDataFor_RecordsOfPatientAPI(p_id){
        ...
        if (!data){
            ...
            for(let i = 0; i < data.length; i++){
                rs.push({'rec_id': data[i].rec_id, 'rec_dease': data[i].rec_dease, 'd_name': data[i].d_name, 'rec_date': data[i].rec_date, 'rec_lastmodified': data[i].rec_lastmodified})
            }
            return rs;
        }
    }
