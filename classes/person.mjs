...
export default class Person {
    ...
    async InsertAccountToDatabase() {
        ...
        const account = await db.get("SELECT * FROM ACCOUNT WHERE acc_un = ?", [
            this.acc_un,
        ]);
        if (account) {
        ...
        try {
            const result = await db.run(
                "INSERT INTO ACCOUNT (acc_un, acc_mk, acc_role) VALUES (?, ?, ?)",
    ...
    async InsertByRole(role) {
        ...
        try {
            ...
            const result3 = await db.run(
                "DELETE FROM ACCOUNT WHERE acc_un = ?",
                [this.acc_un]
            );
    ...
    async GetPrpertiesByIdAndRole(id, role) {
        ...
        const person = await db.get(
            `select * from ${table} where ${prefix}id="${id}"`
        );
        if (person) {
    ...
    static async GetAccountByUsername(username) {
        ...
        const account = await db.get(
            `select * from account where acc_un="${username}"`
        );
