...
import {
    IsDateFormattedCorrect,
    IsTimeFormattedCorrect,
} from "./datetime-helpers.mjs";
...
export function IsDateAndTimeFormatted(
    req: NextApiRequest,
    dateFields: string[],
    timeFields: string[]
) {
    ...
    if (
        IsDatesFomartted(req, dateFields) &&
        IsTimesFomartted(req, timeFields)
    ) {
        return true;
    }
    return false;
